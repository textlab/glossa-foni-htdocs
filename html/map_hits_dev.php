...
$infs = preg_replace('/\)$/', '', $infs); // $infs looks like this imøddom(eiken_ma_01,aaseral_03gm,aaseral_01um,aaseral_02uk),imjøllå(kvinnherad_04gk), ... etc
$infs = preg_replace('/\n*$/', '', $infs);
$pairs = split("\),", $infs);
foreach($pairs as $pair){
  ...
  $places = $pair[1]; // $places is actually informant codes..
  $places = split(",", $places);
  foreach($places as $informant){$informant_location[$informant] = 1;} // informant_location is now array of tid->1 so we need to obtain the lpu
